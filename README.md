...
| 图片头像 | 从相册选择图片，手动选择矩形区域后作为头像 |
| 自定义步骤 | 可自行新增、命名、排序、启停、延时和校准点击步骤 |
...
也可以从 GitHub Releases 下载 APK 后直接发送到平板安装。
...
在设置页打开“自定义点击流程 / 坐标校准”，可以新建一个步骤并填写自己的名称，选择坐标点击动作，设置执行顺序、等待时间和是否在演示模式跳过，然后进入悬浮校准面板选择坐标。这样微信版本或输入法布局变化时，只需调整对应步骤。
...
公开发布时建议使用签名的 Release APK，并在 Release 页面同时提供 SHA-256 校验值。
...
以下截图来自一加平板 Pro（OPD2404）的测试，示例联系人名称不是真实姓名：

![主界面](https://cloudflare-imgbed-chh.pages.dev/file/1785167406413_screenshot-main.png)

![设置与校准入口](https://cloudflare-imgbed-chh.pages.dev/file/1785167412004_screenshot-settings-public.png)

![自定义点击流程](https://cloudflare-imgbed-chh.pages.dev/file/1785167411075_screenshot-automation.png)

## 已测试设备

| 设备 | 屏幕 | 结果 |
|------|------|------|
| 一加平板 Pro（OPD2404） | 2120x3000，竖屏 | 演示流程通过，跳过最后确认 |

## 隐私与发布边界

- 联系人信息、头像和坐标配置保存在设备本地。
- 本项目不要求上传联系人或头像，也不内置 Token。
- GitHub 源码仓库不提交真实头像、联系人配置、日志、签名私钥或个人路径。
- APK 放在 GitHub Releases；发布前请删除平板中的真实测试数据。
